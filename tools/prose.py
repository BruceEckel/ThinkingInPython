...
The companion to tools/markdown.py, which parses a file's *structure* (its
fences, blocks, and headings); this one classifies what is prose *within*
a line. Imported as ``tools.prose``, so no book listing's filename can
shadow it; tools/repo.py's docstring has the history.
"""
