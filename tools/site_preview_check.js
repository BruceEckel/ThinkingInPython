...
//
// `make preview-check` builds the site first and then runs this. No gate
// runs it: it needs node, which nothing else in the repo does, and on its
// first run it needs the network, to install jsdom under build/node/
// (build/ is derived and gitignored, so the repo gains no package.json).
//
...
//
// jsdom lays nothing out, so where the panel sits and how it looks are
// not checked here.
"use strict";
...
function loadJsdom() {
  ...
  if (!fs.existsSync(local)) {
    ...
    execSync("npm install --no-save --no-package-lock --no-audit --no-fund"
             + ` --prefix "${NODE_DIR}" jsdom`, { stdio: "inherit" });
  }
