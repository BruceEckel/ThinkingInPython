# Puts the command the `tip` menu ran into this shell's history, so
...
# function names a scratch file in TIP_MENU_RECORD, runs the real tip,
# and feeds whatever the menu wrote there to `history -s` (bash) or
# `print -s` (zsh), which is immediate. With the variable set, the menu
# leaves the history file alone, so nothing is recorded twice.
tip() {
    local record status line
    record="$(mktemp "${TMPDIR:-/tmp}/tip-menu.XXXXXX")" || {
        command tip "$@"
        return
    }
    TIP_MENU_RECORD="$record" command tip "$@"
    status=$?
    if [ -s "$record" ]; then
