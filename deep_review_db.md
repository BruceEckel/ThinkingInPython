...
- **First generation**, 48 files and roughly 21,000 lines, distilled 2026-08-09
  ...
  `git show 8660e7c1^:deep_review/~19_Concurrency.md`.
- **Second generation**, 47 chapter files written 2026-08-11 by a full sweep on
  ...
  `git show bb81711a:deep_review/~03_Containers.md`.
...
- **02_Tour** gained exercise 6, a prediction exercise for negative floor
  division (`-9 // 4` and `-9 % 4`), with its solution in `Solutions/02_Tour.md`.
  The chapter's sharpest fact for a C or Java reader had no exercise.
- **06_Modules_and_Packages** gained `exporting.py` and `star_import.py`
...
- **18_Performance**: the NumPy section became a real tested listing.
...
- **30_Observer**: exercise 2 was reworded to describe the game
  `Solutions/30_Observer.md` actually implements. The exercise had described a
...
**34_Composite_and_Interpreter: the three-walker paragraph is not a misplaced
...
**03_Containers: the `deque` threshold stays at `* 20`.** A first-generation
...
**09_Class_Attributes: the `ty` augmented-assignment gap closed.** `ty` 0.0.70
...
`Solutions/09_Class_Attributes.md` and `Solutions/12_Data_Classes_as_Types.md`
