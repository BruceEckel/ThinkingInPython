...
because `Proxy4` declares no `level` and no `__setattr__()` to accept one,
...
The separate implementation hierarchy that *GoF Design Patterns* uses earns its keep when you do not control the implementing code.
