...
Iterators are easy to test by collecting them into a list and comparing. Worth
covering: the generated sequences, the empty edge cases, that a custom iterable
can be iterated more than once, that a function written against `Iterable` works
on every source, and that the type-checking wrappers raise on a mismatch.
