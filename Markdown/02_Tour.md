...
When something goes wrong at runtime, Python raises an *exception*: a signal
that interrupts the normal flow and travels up the call stack.
Dividing by zero, indexing past the end of a list, or opening a missing file
each raises an exception.
If nothing handles that exception, the program stops and prints a traceback.

A `try`/`except` block handles an exception so the program can continue:
...
If a matching `except` clause exists, control jumps to it, binding the
exception object to the name after `as`.
Execution then continues past the block instead of stopping.
Each `except` names the exception type it handles, so an unrelated error is not
caught by accident.

Raise an exception using `raise`.
A `finally` clause runs whether or not an exception occurred, which makes it the place for cleanup:
